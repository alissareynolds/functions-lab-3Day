...
const displayParams = (parameter1, parameter2) => console.log(`The first parameter is ${parameter1}. The second parameter is ${parameter2}.`);
...
greeting('ALissa', 'Reynolds', sayHello);
...
const pricesPlusTax = (prices, callback) => {
    for(let i = 0; i < prices.length; i++) {
        totalCost.push(prices[i] * .20 + prices[i]);
    }
    callback(totalCost);
} 
...
pricesPlusTax(prices, printArray)
function printArray(totalCost){
...
function multiplyingFactory(firstNum) {
